...
export default function NewTradePage() {
  ...
  // Auto-calculate RR when entry, sl, target are filled
  const calcRR = () => {
    ...
    <div className="p-8 page-transition max-w-5xl">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <Card className="p-6 mb-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <FormGroup label="Risk:Reward Ratio">
              <input type="number" className="inp" placeholder="Auto or manual" step="0.01"
        ...
        <Card className="p-6 mb-5">
          ...
          <p className="text-xs text-ink-400 mb-4">Jo mistakes ki ho wo select karo — yahi data future improvement ke liye kaam aayega</p>
          <div className="grid grid-cols-2 gap-2">
            {MISTAKES.map(m=>{
              const sel = mistakes.includes(m.id);
              return (
                <button key={m.id} type="button" onClick={()=>toggleMistake(m.id)}
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-ink-400 mt-0.5">{m.tip.slice(0,50)}...</div>
                  </div>
                </button>
              );
            })}
          ...
          {mistakes.length > 0 && (
            <div className="mt-3 p-3 bg-red/5 border border-red/15 rounded-xl">
              <div className="text-xs text-red font-semibold">⚠️ {mistakes.length} mistake{mistakes.length>1?"s":""} ki — next trade mein improve karo!</div>
            </div>
          )}
        </Card>
        ...
        <Card className="p-6 mb-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <FormGroup label="Rules Follow Kiye? (kya, kaise)">
              <textarea className="inp" rows={2}
                placeholder="e.g. SL aur target dono set kiye the, lot size fixed rakha — ya nahi kiya aur kyun nahi..."
                value={form.rulesFollowed} onChange={e=>set("rulesFollowed",e.target.value)} />
            </FormGroup>
            <FormGroup label="Tags (comma-separated)">
              <input className="inp" placeholder="e.g. trend, news, london-session, good-setup"
