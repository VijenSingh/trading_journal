...
import {
  ...
  ResponsiveContainer, ScatterChart, Scatter, Cell, RadarChart,
    ...
    <div className="bg-bg-700 border border-white/10 rounded-xl p-3 text-xs font-mono shadow-card">
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} style={{ color: p.color || "#E8EDF5" }}>
          {p.name}: {typeof p.value === "number" && (p.name.includes("P&L") || p.name.includes("pnl"))
...
export default function AnalyticsPage() {
  ...
  trades.forEach(t => {
    const s = (t.strategy && t.strategy.trim()) ? t.strategy.trim() : "Unknown";
    if (!stratPnl[s]) stratPnl[s] = { pnl: 0, trades: 0, wins: 0 };
    stratPnl[s].pnl += t.pnl;
    stratPnl[s].trades++;
    if (t.pnl > 0) stratPnl[s].wins++;
  });
  ...
  trades.forEach(t => { const s = t.session || "Unknown"; sessPnl[s] = (sessPnl[s] || 0) + t.pnl; });
    ...
    <div className="p-8 page-transition">
      ...
      {trades.length === 0 ? (
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <StatCard label="Win Streak" value={a.longestWinStreak} color="green" sub="Longest consecutive wins" />
            <StatCard label="Loss Streak" value={a.longestLossStreak} color="red" sub="Longest consecutive losses" />
            <StatCard label="Best Pair" value={a.bestPair.split(" ")[0]} color="green" />
            <StatCard label="Worst Pair" value={a.worstPair.split(" ")[0]} color="red" />
          </div>

          {/* Cumulative chart + Radar */}
                    ...
                    <div key={s.name} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-6 bg-bg-700 rounded-lg overflow-hidden relative">
                        <div className="h-full rounded-lg transition-all" style={{
                          width:`${Math.min(100,Math.abs(s.pnl)/Math.max(...stratData.map(x=>Math.abs(x.pnl)))*100)}%`,
                  ...
                  <div key={m.id} className={cn(
                    ...
                    <div className="mt-2 h-1 bg-bg-600 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{
                        width:`${pct}%`, background:count>0?"#FF4560":"#00E676", opacity:0.7
                      }}/>
                    </div>
                  </div>
                );
