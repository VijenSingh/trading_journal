...
export async function GET(req: NextRequest) {
  try {
    ...
    const doc = await GoalModel.findOne({ periodType, periodKey }).lean();
    return NextResponse.json({ success: true, data: doc || { periodType, periodKey, targetPnl: 0, maxLossLimit: 0 } });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    await connectDB();
    const { periodType, periodKey, targetPnl, maxLossLimit } = await req.json();
    if (!periodType || !periodKey) {
      return NextResponse.json({ success: false, error: "periodType and periodKey required" }, { status: 400 });
    }
