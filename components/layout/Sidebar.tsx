...
import {
  ...
  Building2, Menu, X, Calculator, Landmark, ClipboardCheck, Image, Award,
} from "lucide-react";
    ...
    <>
      {nav.map(({ href, icon: Icon, label }) => {
        ...
        return (
          ...
          >
            <Icon size={16} className={active ? "text-purple" : "text-ink-400"} />
            {label}
            {label === "New Trade" && (
              <span className="ml-auto text-[10px] bg-purple/10 text-purple border border-purple/20 rounded px-1.5 py-0.5 font-mono">+</span>
            )}
          </Link>
        );
      })}
...
export default function Sidebar() {
  ...
  const Logo = () => (
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple to-pink flex items-center justify-center flex-shrink-0 shadow-glow-purple">
    ...
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer */}
          <div className="relative w-[280px] h-full bg-bg-900 border-r border-black/[0.05] flex flex-col animate-slide-up">
            <div className="px-4 py-4 border-b border-black/[0.05] flex items-center justify-between">
              <Logo />
              <button
