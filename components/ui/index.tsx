...
export function Card({ className, children }: { className?: string; children: ReactNode }) {
  return (
    <div className={cn(
      "bg-white/70 backdrop-blur-xl border border-black/[0.06] rounded-2xl shadow-[0_4px_24px_rgba(124,58,237,0.06)]",
      className
    )}>
      {children}
    </div>
...
const badgeClasses: Record<BadgeVariant, string> = {
  ...
  gray:   "bg-ink-500/15 text-ink-300 border-ink-400/20",
};
...
const btnVariants = {
  primary: "bg-gradient-to-r from-purple to-pink text-white hover:brightness-110 font-semibold shadow-glow-purple",
  danger:  "bg-red/10 text-red border border-red/20 hover:bg-red/20",
  ghost:   "bg-bg-700 text-ink-200 border border-black/[0.06] hover:bg-bg-600 hover:text-ink-100",
  outline: "bg-transparent text-ink-200 border border-black/[0.1] hover:border-black/[0.2] hover:text-ink-100",
};
...
export function Divider() {
  return <div className="h-px bg-black/[0.05] my-6" />;
}
